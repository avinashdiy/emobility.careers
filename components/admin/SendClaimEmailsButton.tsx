...
export function SendAllClaimEmailsForm({ pendingCount }: { pendingCount: number }) {
  ...
  return (
    <form action={formAction}>
      {state.message && (
        <div role="alert" className={`mb-3 rounded-md p-3 text-sm ${state.ok ? "bg-emce-light-soft text-emce-darkest" : "bg-emce-red-light text-emce-red"}`}>
          {state.message}
        </div>
      )}
      <Button
...
export function ResendClaimForm({ userId, email }: { userId: string; email: string }) {
  ...
  return (
    <form action={formAction} className="inline-flex items-center gap-2">
      ...
      {state.message && (
        <span className={`text-xs ${state.ok ? "text-emce-mid" : "text-emce-red"}`}>{state.message}</span>
      )}
