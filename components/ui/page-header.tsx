    ...
    >
      <div className="min-w-0 flex-1">
        {backHref && (
          <Link
            href={backHref}
            className="mb-1 inline-flex items-center gap-1 text-hint font-bold text-emce-dark hover:underline dark:text-emce-mid"
          >
            ← {backLabel ?? "Back"}
          </Link>
        )}
        {eyebrow && (
          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-emce-mid-muted dark:text-emce-mid">
            {eyebrow}
          </p>
        )}
        <h1 className="text-2xl font-extrabold leading-tight tracking-tight text-emce-text dark:text-foreground md:text-[28px]">
          {titleNode}
        </h1>
        {subtitle && (
          <p className="mt-1 text-sm text-emce-text-sec dark:text-muted-foreground">{subtitle}</p>
        )}
      </div>
...
export function SectionTitle({
  ...
  return (
    <div className={cn("flex flex-wrap items-start justify-between gap-2", className)}>
      <div className="min-w-0 flex-1">
        <h2 className="text-section text-emce-text dark:text-foreground">{title}</h2>
        {description && (
          <p className="mt-0.5 text-hint text-emce-text-sec dark:text-muted-foreground">{description}</p>
        )}
      </div>
